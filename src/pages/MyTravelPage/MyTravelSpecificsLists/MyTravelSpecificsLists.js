    ...
    return (
        <div>
            ...
            >
                ...
                {isEditClicked && (
                    <button
                        className="edit-specific-empty-button"
                        onClick={() =>
                            handleEditSpecificsClick({ setIsTravelCreate })
                        }
                    ></button>
                )}
            </div>
    ...
    return (
        <div>
            <div className="travel-card-header">
                ...
                <span className="day">
                    {formatDate(start)}
                    {getDayOfWeek(start, index)}
                </span>
            </div>
            <div className="travel-card">
                {isEditClicked && (
                    <button
                        className="edit-specific-extra-button"
                        onClick={() => handleEditSpecificsClick({ index })}
                    ></button>
                )}
                ...
                {spot2 && (
                    <SpotDisplay num="2" spot={spot2.title} distance={second} />
                )}
                {spot3 && (
                    <SpotDisplay num="3" spot={spot3.title} distance={third} />
                )}
...
function MyTravelSpecificsLists({
        ...
        const fetchSpecificData = async () => {
            try {
                ...
                if (response.status === 200) {
                    ...
                    console.log(TAG + "-selectedTID ", selectedTID);
                }
            } catch (error) {
    ...
    if (
        Object.values(travelSpecificData.courses).length != 0 ||
        travelSpecificData.courses.length != 0
    ) {
    ...
    useEffect(() => {
        console.log("현재 travelCourse 값:", travelCourse);
    }, [travelCourse]);
    ...
    return (
        <div>
            ...
            <div className="my-travel-specific-lists">
                ...
                {/* {console.log("디버깅", travelCourse[0].spot1.title == null)} */}
                {console.log("디버깅", travelSpecificData)}
                {travelCourse.length == 1 && !travelCourse[0].spot1.title ? (
                    <TravelEmptyCard
                        start={travelSpecificData.start_date}
                        end={travelSpecificData.end_date}
                ...
                {isEditClicked ? (
                    <>
                        <button
                            className="edit-specific-plus-button"
                            onClick={handlePlusClick}
                        ></button>
                        <button
                            className="edit-specific-minus-button"
                            onClick={handleEditClick}
                        ></button>
                    </>
                ) : (
                    <button
                        className="edit-specific-button"
                        onClick={handleEditClick}
                    ></button>
                )}
            </div>
